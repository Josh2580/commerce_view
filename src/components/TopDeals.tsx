...
export const TopDeals = () => {
  ...
  const handleResize = () => {
    ...
    if (isLargeScreen) {
      setDisplayItems(featuredProducts.slice(0, 5)); // Show up to 10 items on large screens
    } else if (isMediumScreen) {
      setDisplayItems(featuredProducts.slice(0, 4)); // Show up to 10 items on large screens
    } else {
      setDisplayItems(featuredProducts.slice(0, 3)); // Show only 4 items on small screens
    }
  };
