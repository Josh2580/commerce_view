...
export const NewCategoryProducts: React.FC = () => {
  ...
  const handleResize = () => {
    ...
    if (isLargeScreen) {
      setDisplayItems(newProducts.slice(0, 10)); // Show up to 10 items on large screens
    } else if (isMediumScreen) {
      setDisplayItems(newProducts.slice(0, 8)); // Show up to 10 items on large screens
    } else {
      setDisplayItems(newProducts.slice(0, 6)); // Show only 4 items on small screens
    }
  };
  ...
  return (
    <div className=" bg-white my-responsive card-space">
      <div className="flex justify-between ">
        <h2 className="batch-head">New Products</h2>
